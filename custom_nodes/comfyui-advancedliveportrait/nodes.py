...
class LP_Engine:
    ...
    def get_detect_model(self):
        if self.detect_model is None:
            ...
            try:
                # Try to add the required class to safe globals
                torch.serialization.add_safe_globals(['ultralytics.nn.tasks.DetectionModel'])
            except AttributeError:
                # Fallback for older PyTorch versions that don't have add_safe_globals
                pass
            
            # Alternative approach using context manager if the above doesn't work
            try:
                with torch.serialization.safe_globals(['ultralytics.nn.tasks.DetectionModel']):
                    self.detect_model = YOLO(model_path)
            except (AttributeError, TypeError):
                # Fallback: try with weights_only=False (less secure but functional)
                try:
                    # This requires modifying how YOLO loads internally, so we'll use a different approach
                    import ultralytics.nn.tasks
                    torch.serialization.add_safe_globals([ultralytics.nn.tasks.DetectionModel])
                    self.detect_model = YOLO(model_path)
                except:
                    # Last resort: patch torch.load temporarily
